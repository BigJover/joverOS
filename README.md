...
| `save as workflow 1` → `workflow 1` / `workflows` / `delete workflow 1` | **record the apps you have open, replay them by name** — each app's own session restore brings back its windows |
...
| `disk space` / `wifi` / `sound` / "why is my mac slow" / "report.pdf won't open" | **diagnose** the problem, layer by layer |
